...
export function EstimatesList() {
  ...
  const cancel = async (e) => {
    if (!window.confirm(`Cancel estimate ${e.estimate_no}?`)) {
      return;
    }
      ...
      <div className="grid lg:grid-cols-2 gap-3">
        ...
        {items.map((e) => {
          ...
          return (
            <GlassCard
              key={e.id}
              data-testid={`estimate-card-${e.estimate_no}`}
              className="!p-4 sm:!p-5"
            >
              <div className="flex items-start justify-between mb-2 gap-2">
                <div className="min-w-0">
                  <div className="text-[10px] font-mono-receipt text-white/60">
                    {e.estimate_no}
                  </div>
              ...
              <div className="grid grid-cols-3 gap-2 mt-3 text-xs">
                ...
                <MiniStat
                  label="Grand"
                  value={formatRupee(e.grand_total)}
                />
                <MiniStat
                  label="Remaining"
                  value={formatRupee(e.remaining)}
                />
              </div>
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                {e.status === "active" && (
                  <>
                    <button
                      data-testid={`estimate-confirm-${e.estimate_no}`}
                      onClick={() => confirmDispatch(e)}
                      disabled={busy === e.id}
                      className="btn-primary rounded-full px-3 py-2 text-xs uppercase tracking-[0.18em] inline-flex items-center gap-1.5"
                    >
                    ...
                    <button
                      onClick={() =>
                        navigate(`/estimates/${e.id}/edit`)
                      }
                      className="rounded-full glass px-3 py-2 text-xs uppercase tracking-[0.18em] hover:bg-white/10"
                    >
                      Edit
                    </button>
                  </>
                ...
                {e.status === "active" && (
                  <button
                    onClick={() => cancel(e)}
                    className="rounded-full bg-red-500/10 border border-red-500/30 text-red-300 px-3 py-2 text-xs uppercase tracking-[0.18em] hover:bg-red-500/20"
                  >
                    Cancel
                  </button>
                )}
...
export function EstimateForm({ editMode = false }) {
  ...
  useEffect(() => {
    cachedGet("products:full", "/products", {
      ttl: 30000,
    })
      .then(setProducts)
      .catch(() => {});

    cachedGet("customers:list", "/customers", {
      ttl: 30000,
    })
      .then(setCustomers)
      .catch(() => {});
      ...
      api
        ...
        .then((r) => {
          ...
          Promise.all(
            (e.items || []).map((it) =>
              api
                .get(`/products/${it.product_id}`)
                .then((rr) => ({
                  ...it,
                  size_preset: rr.data.size_preset,
                  image:
                    it.image ||
                    rr.data.images?.[0],
                }))
  ...
  /* ---------------------------------------------
     DASHBOARD SCAN SR -> ESTIMATE
     
     Dashboard sends:
       location.state.preselectProduct

     This automatically adds that product to
     the estimate without changing existing
     manual Add / Scan / SR functionality.
  --------------------------------------------- */
  ...
  useEffect(() => {
    ...
    /*
      ...
      This prevents the same scanned product from
      being automatically added again when the user
      navigates back into the form.
    */
  ...
  const grandTotal =
    itemTotal + (Number(delivery) || 0);

  const remaining = Math.max(
    0,
    grandTotal - (Number(advance) || 0)
  );
  ...
  const save = async () => {
    ...
    const cleaned = items.map((it) => ({
      ...
      sizes: Object.fromEntries(
        Object.entries(it.sizes || {}).filter(
          ([, v]) => v > 0
        )
      ),
    ...
    if (
      cleaned.some(
        (it) =>
          Object.keys(it.sizes).length === 0
      )
    ) {
      return setError(
        "Each item needs at least one size with quantity"
      );
    }
    ...
    <div className="max-w-3xl mx-auto space-y-4 pb-8">
      ...
      <GlassCard>
        <div className="grid sm:grid-cols-2 gap-3">
          <label className="block sm:col-span-2">
            <span className="text-xs text-white/70 mb-1.5 inline-block">
          ...
          <label className="block">
            ...
            <input
              data-testid="estimate-customer-name"
              value={customerName}
              onChange={(e) =>
                setCustomerName(
                  e.target.value
          ...
          <label className="block">
            ...
            <input
              data-testid="estimate-customer-phone"
              value={customerPhone}
              onChange={(e) =>
                setCustomerPhone(
                  e.target.value
      ...
      <GlassCard>
        ...
        <div className="flex gap-2 mb-3">
          ...
          <button
            onClick={loadBySr}
            className="btn-primary rounded-full px-5 py-2.5 text-xs uppercase tracking-[0.2em]"
          >
            Add by SR
          </button>
        </div>
            ...
            >
              <div className="flex items-start justify-between mb-2 gap-3">
                ...
                <button
                  ...
                  aria-label="Remove"
                  className="text-white/45 hover:text-red-400 p-1"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
      ...
      <GlassCard>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-4">
          <MiniStat
            label="Item Total"
            value={formatRupee(itemTotal)}
          />

          <MiniStat
            label="Delivery"
            value={formatRupee(delivery)}
          />
      ...
      <div className="flex justify-end gap-2">
        <button
          onClick={() => navigate(-1)}
          className="rounded-full glass px-5 py-2.5 text-xs uppercase tracking-[0.18em] hover:bg-white/10"
        >
          Cancel
        </button>
...
function ProductPickerModal({
  ...
  return (
    ...
    >
      <div
        className="modal-shell w-full max-w-xl rounded-2xl border shadow-2xl fade-up
          ...
          {products.map((p) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[10px] font-mono-receipt text-[var(--sca-text-muted)] dark:text-white/60">
                  {p.sr_number}
                </div>
              ...
              <div className="text-right flex-shrink-0">
                ...
                <div className="text-[10px] text-[var(--sca-text-muted)] dark:text-white/55 mt-0.5">
                  {p.size_preset}
                </div>
              </div>
            </button>
